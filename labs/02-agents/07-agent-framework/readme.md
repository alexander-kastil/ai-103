...
> **Instructor note**: A complete, verified solution is in the `solution-py` folder (sibling of `Python`). It was run end-to-end against Azure Foundry and the agent correctly invokes the custom tool. Corrections applied versus the original code blocks below:
> - Added `load_dotenv()` (and the `from dotenv import load_dotenv` import). `python-dotenv` is listed in `requirements.txt` and the lab uses a `.env` file, but the starter code never loaded it, so `PROJECT_ENDPOINT` and `MODEL_DEPLOYMENT_NAME` would be unset when running outside an environment that already exports them.
> - Fixed a typo in the agent instructions string: `'Expense Claim`and a body` was changed to `'Expense Claim' and a body` (stray backtick and missing space).
...
For this exercise, you'll use starter code that will help you connect to your Foundry project and create an agent that can process expenses data.
...
1. Near the bottom of the file, find the comment **Create a tool function for the email functionality**, and add the following code to define a function that your agent will use to send email:
...
1. Find the comment **Use the agent to process the expenses data**, and add the following code:
