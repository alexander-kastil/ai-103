...
> **Instructor note**: A complete, runnable reference implementation lives in `solution-py` (sibling of `Python`). It was verified end to end against Foundry. Corrections applied versus the starter code:
> - Removed the module-level `routing_agent = _get_initialized_routing_agent_sync()` line at the bottom of `routing_agent/agent.py`. It called `asyncio.run()` at import time and tried to reach the remote agents before the servers were listening. The routing agent is created in `routing_agent/server.py` during the FastAPI lifespan, so this line is redundant and harmful.
> - Added `requests` to `requirements.txt` (`client.py` imports it).
> - Replaced double-quoted environment lookups nested inside f-strings (`f"...{os.environ["SERVER_URL"]}..."`) with single-quoted keys for compatibility.

In this exercise, you'll use Microsoft Foundry Agent Service with the A2A protocol to create simple remote agents that interact with one another. These agents will assist technical writers with preparing their developer blog posts. A title agent will generate a headline, and an outline agent will use the title to develop a concise outline for the article. The agents are built with the new Foundry **prompt agent** pattern: each agent is created with `create_version` and a `PromptAgentDefinition`, and conversations run through the OpenAI-compatible **conversations** and **Responses** APIs. Let's get started.
...
1. In the Model Catalog, locate the **gpt-5.4** model (you can use the search bar to find it quickly).

1. Select **Deploy** next to the gpt-5.4 model.

1. Configure the deployment settings:
   - **Deployment name**: Enter a name like "gpt-5.4"
...
1. Select **Deploy to Microsoft Foundry** in the bottom-left corner.
    ...
    > **Important**: This lab deploys **two** models. The title and outline agents read `MODEL_DEPLOYMENT_NAME` (default `gpt-5.4`); they use no tools, so `gpt-5.4` works fine. The **routing agent** relies on the **Function** tool to delegate work, and not every model supports it. At the time of writing, `gpt-5.4` does **not** support the Function tool in Foundry Agent Service, while `gpt-4.1`, `gpt-4o`, `gpt-5`, and `gpt-5.2` do. So the routing agent reads a separate `ROUTING_MODEL_DEPLOYMENT_NAME` (default `gpt-5.2`, which is Function-capable). Deploy both `gpt-5.4` and `gpt-5.2` in your project. Always confirm tool support for your chosen model and region in [Tool support by region and model](https://learn.microsoft.com/azure/foundry/agents/concepts/tool-best-practice#tool-support-by-region-and-model).
...
1. Right-click the name of the project deployment and select **Copy Project Endpoint**. You'll need this URL to connect your agent to the Foundry project in the next steps.
    ...
    ![Screenshot of copying the project endpoint in the Foundry Toolkit VS Code extension.](../../_images/vs-code-endpoint.png)

## Open the starter code

For this exercise, you'll use starter code that will help you connect to your Foundry project and create remote agents that interact using the A2A protocol.
...
1. Find the comment **Create the project client** and add the following code to connect to the Foundry project and obtain the OpenAI-compatible client:
   ...
   self.agent = None
    ...
    The `project` client manages agent creation and versioning, while the `openai` client (obtained with `get_openai_client()`) runs conversations and responses.

1. Find the comment **Create the title agent** and add the following code to create a prompt agent with `create_version`:
...
1. Find the comment **Create a conversation for the chat session** and add the following code to start a conversation:
   ...
   conversation = self.openai.conversations.create()
    ...
    Conversations replace the threads and messages used by the older Assistants-style API. A conversation stores the full stream of items (messages, tool calls, and tool outputs) and retains context across turns.

1. Under the comment **Send the user message and run the agent**, add the following code to generate a response with the Responses API:
   ...
   response = self.openai.responses.create(
    ...
    Responses replace the runs and polling loop of the older API. The `agent_reference` tells the Responses API which agent version to run. The agent's text is available directly through `response.output_text`, which the rest of the file returns.
...
1. Open the **routing_agent/agent.py** file in the code editor.
    ...
    - Starts a conversation and sends the user message to its prompt agent with the Responses API.
    - The agent evaluates the request and emits a `send_message` function call for the best-matching remote agent.
    ...
    The routing agent submits the remote agent's result back as a `function_call_output` item and loops until the agent returns a final answer, which is returned to the user.

    The routing agent is defined as a prompt agent whose only tool is the `send_message` function. Notice that the `send_message` method is async and must be awaited so the remote agent call completes before its output is returned to the model.
...
If you've finished exploring Microsoft Foundry Agent Service, you should delete the resources you have created in this exercise to avoid incurring unnecessary Azure costs.
