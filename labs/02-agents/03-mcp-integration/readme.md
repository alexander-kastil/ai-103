...
> **Instructor note**: A complete, verified solution is in the `solution-py` folder (sibling of `Python`). It was run end-to-end against Azure Foundry. Two corrections were applied to the code blocks below and are reflected in `solution-py`:
>
> 1. In `agent.py`, a single approval round is not enough. Because `require_approval="always"`, each MCP call produces a new approval request, so the approval handling must loop until the response contains no more approval requests before printing the final answer.
> 2. In `client.py`, the MCP server must be launched with `sys.executable` instead of the literal `"python"`. Using `"python"` resolves to the system interpreter, which does not have the lab packages installed, and the server fails to start.
...
1. Find the comment **Process any MCP approval requests that were generated** and add the following code:
   ...
   print(f"\nAgent response: {response.output_text}")
    ...
    This code listens for any MCP approval requests in the agent's response and automatically approves them. Because the tool is configured with `require_approval="always"`, each MCP call produces a new approval request, so the loop continues until the response has no more approval requests, then prints the final answer.
...
1. In the **connect_to_server** method, change the `command` value in `server_params` from `"python"` to `sys.executable` so the server is launched with the same interpreter that runs the client. Add `import sys` at the top of the file. Using the literal `"python"` resolves to the system interpreter, which does not have the lab packages installed, and the server fails to start.
...
1. Under the comment **Start the MCP server**, add the following code:
