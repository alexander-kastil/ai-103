...
1. Ensure the **New Foundry** toggle is set to *On*.
    ...
    ![Screenshot of the New Foundry toggle.](../../_images/ai-foundry-toggle.png)
...
2. You may be prompted to create a new project before continuing to the New Foundry experience. Select **Create a new project**.
    ...
    ![Screenshot of the prompt to create a new project.](../../_images/ai-foundry-new-project.png)
...
2. In the **Set variable** node editor, enter a name for a new variable, such as *SupportTickets*.
    ...
    ![Screenshot of creating a new variable in the Set variable node.](../../_images/node-new-variable.png)
...
1. In the editor, under **Details**, select the **Parameters** button near the model name.
    ...
    ![Screenshot of the Parameters button in the agent editor.](../../_images/agent-parameters.png)
...
1. Observe the workflow as it processes each support ticket in sequence. Review the messages generated by the workflow in the chat window.
    ...
    You should see some output indicating that billing issues are being escalated, while technical and general issues receive drafted responses.

## Use your workflow in a client application

Now that you've built and tested your workflow in the Foundry portal, you can also invoke it from your own code using the Azure AI Projects SDK.
...
3. Copy the value of the **AZURE_EXISTING_AIPROJECT_ENDPOINT** variable, which is the endpoint URL for your Foundry project.
...
6. Open the **.env** file, replace the **your_project_endpoint** placeholder with the endpoint for your project. Use **Ctrl+S** to save the file after making these changes.
...
2. Find the comment **Connect to the agents client**, and add the following code:
...
4. Find the comment **Create a conversation and run the workflow**, and add the following code:
...
5. Find the comment **Process events from the workflow run**, and add the following code:
...
6. Find the comment **Clean up resources**, and enter the following code:
...
1. Wait a moment for the workflow to process the tickets. You should see output in the console indicating the progress of the workflow.

1. When the workflow completes, you should see output showing the classification of each ticket and the recommended response or escalation.
