...
> **Instructor note**: A complete, verified solution is in the `solution-py` folder (sibling of `Python`). It was run end-to-end against Foundry with `gpt-5.4`. Two corrections relative to the steps below:
>
> - The starter `.env` uses the variable names `AZURE_AI_PROJECT_ENDPOINT` and `AZURE_AI_MODEL_DEPLOYMENT_NAME`. The bare `AzureAIAgentClient(credential=credential)` reads those names automatically via the `AZURE_AI_` prefix. The "Open the starter code" step refers to `MODEL_DEPLOYMENT_NAME`, which is the prose name, not the actual `.env` key.
> - `solution-py` instead uses `PROJECT_ENDPOINT` / `MODEL_DEPLOYMENT_NAME` and passes them explicitly to `AzureAIAgentClient(project_endpoint=..., model_deployment_name=..., credential=...)` to match the convention used across the other demos in this repo. Either approach works; pick one and keep the `.env` keys consistent with it.
...
Now you're ready to create the agents for your multi-agent solution!
