...
def next_visible_event(location: str) -> str:
    """Finds the next visible astronomical event for a given continent."""
    loc = location.lower().strip().replace(" ", "_")

    now = datetime.now()
    today_md = now.month * 100 + now.day

    # Events visible from this location, sorted by date (set up at module load).
    visible = [e for e in EVENTS if loc in e[4]]

    if not visible:
        known = sorted({l for e in EVENTS for l in e[4]})
        return json.dumps({
            "error": f"No events found for location '{location}'. Known locations: {', '.join(known)}"
        })

    # The next event at or after today; if the year's events have passed, wrap to the first of next year.
    upcoming = [e for e in visible if e[2] >= today_md]
    event = (upcoming or visible)[0]
