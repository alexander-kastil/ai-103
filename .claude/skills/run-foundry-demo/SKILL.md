...
description: >-
  Guide the user through running a Foundry demo in modules/02-agents. Prepares
  the environment (config, venv, deps) and gives the user the terminal commands
  to run. Does not run interactive scripts directly. Errors are discussed and
  resolved collaboratively.
  Triggers on: "run demo", "run the demo", "run this demo", "start demo", "execute demo".
---
...
Look for `provision.azcli` in the demo's module folder (e.g., `modules/02-agents/04-foundry-iq/provision.azcli`). If present, run it in Bash before doing anything else. It creates all required Azure resources and updates `.claude/deploy.config`.
...
Identify the target `*-py/` folder under `modules/02-agents/`. If not clear from context, ask.
...
After resolving any issues, update the `readme.md` in the parent folder of the demo (e.g. `modules/02-agents/04-foundry-iq/readme.md`) with:
...
Before proceeding, confirm `gpt-5.4` fits the demo. Ask the user and propose alternatives via `mcp__azure-deploy__foundry` if the demo needs vision, audio, embeddings, or a fine-tuned model.
...
- Provision scripts go in the module folder, not the `*-py/` subfolder
